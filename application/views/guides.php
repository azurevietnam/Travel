...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#tour-list').load("<?php echo base_url()?>guides/loadPost/<?php echo $CATE_ID?>", {'group_no':track_load}, function() {track_load++;}); //load first group

        $(window).scroll(function() { //detect page scroll

            if($(window).scrollTop() + $(window).height() >= $(document).height()-600)  //user scrolled to bottom of the page?
            {
                if (track_load <= pages && loading == false) //there's more data to load
                {
                    loading = true; //prevent further ajax loading

                    //load data from the server using a HTTP POST request
                    $.post('<?php echo base_url()?>guides/loadPost/<?php echo $CATE_ID?>', {'group_no': track_load}, function (data) {
...
<div class="page-title-container">
    <div class="container">
        ...
        <ul class="breadcrumbs pull-right">
            ...
            <li><a href="<?php echo base_url()?>guides/guides_list/<?php echo $CATE_ID?>"><?php if($CATE_ID=='all'){echo $this->lang->line('all');}
